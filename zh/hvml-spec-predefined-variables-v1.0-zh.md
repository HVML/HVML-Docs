...
该方法根据所传入的数据返回一个有符号长整数；若不传入参数，则视作 `undefined`。

若在构造过程中出现向上溢出错误，则该方法将返回有符号长整数表达范围中的最大值；若在构造过程中出现向下溢出错误，则该方法将返回有符号长整数表达范围中的最小值。
...
该方法根据所传入的数据返回一个无符号长整数；若不传入参数，则视作 `undefined`。

若在构造过程中出现向上溢出错误，则该方法将返回无符号长整数表达范围中的最大值；若在构造过程中出现向下溢出错误，则该方法将返回无符号长整数表达范围中的最小值（`0UL`）。
...
$DATA.ulongint(123)
    // 123UL
$DATA.ulongint(-123L)
    // 18446744073709551493UL
```

#### 3.7.37) `bigint` 方法

使用数据构建一个大整数（任意精度整数，有符号）。
...
该方法根据所传入的数据返回一个大整数；若不传入参数，则视作 `undefined`。

**异常**

该方法可产生如下异常：

- `TooLarge`：数值太大，超出存储限制；静默求值时返回 `0N`。
- `Overflow`：对参数进行数值化处理时出现向上溢出错误；静默求值时返回 `0N`。
- `UnderFlow`：对参数进行数值化处理时出现向下溢出错误；静默求值时返回 `0N`。
...
$DATA.bigint
    // 0UL
$DATA.bigint('1234567890123456789012345678901234567890')
...
该方法根据所传入的数据返回一个双精度浮点数；若不传入参数，则视作 `undefined`。
...
该方法根据所传入的数据返回一个长双精度浮点数；若不传入参数，则视作 `undefined`。
...
$DATA.longdouble
    // 0.
$DATA.longdouble('235.25')
    // 235.25FL
$DATA.longdouble(123)
    // 123.FL
$DATA.longdouble(-123L)
    // -123.FL
```
...
格式化数值及字符串数据，格式字符串使用类似 C 语言的转换指示符（coversion specifier）。
...
当使用转换指示符 `s`、`]` 之一时，由于平台功能所限，其行为可能不同：
   - 在不支持 `m` 修饰符的平台上，在不指定 `*` 或者宽度的情形下，对 `s`、`]` 转换指示符将使用尺寸为 1024 字节（含结尾空字符在内）的缓冲区。
   - 在支持 `m` 修饰符的平台上，将在不指定 `*` 或者宽度的情形下自动添加该修饰符。

当使用转换指示符 `s`、`c`、`]` 之一时，若转换后的字符构成有效的 UTF-8 编码字符串，则生成字符串，否则生成字节序列。
