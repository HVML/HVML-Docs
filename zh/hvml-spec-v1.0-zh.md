...
第一，在广泛使用的 JSON 表述方法之上，HVML 使其具有了动态处理能力以及参数化表述数据的能力。JSON 是一种人机共读的数据表述形式，可在数值、字符串、数组、字典等基本数据单元的基础上表述复杂对象。HVML 扩展了 JSON 表述方法，使之支持更多数据类型，并通过使用动态值和原生实体这两类动态数据，HVML 定义了从底层系统获得数据或者实现某种功能的方法，并在此基础上实现了灵活的表达式求值能力。这可以帮助我们利用已有的系统能力，也可以帮助开发者快速扩展 HVML 程序的功能和能力。
...
第二，HVML 使用类似 XML 的标记语言来定义程序结构。HVML 定义了为数不多的十多个动作标签，可用来定义变量，操作数据或者控制程序的执行路径。和这些动作标签配合，HVML 使用介词属性和副词属性来定义动作依赖的源数据、目标数据以及执行条件，从而获得更加贴近自然语言的程序描述方式。这一方面降低了开发者的学习门槛，另一方面可以大幅提高代码的可读性。
...
第三，数据驱动编程。在 HVML 中，一切编程行为围绕要处理的数据进行，比如选择数据，在数据上迭代，执行规约操作，观察数据的变化等等，甚至我们通过更改数据来操控某个功能的实现，比如增加一个定时器，我们不需要调用某个 API，而是在一个集合中新增一个数据项。我们将这种编程方式称为数据驱动的编程（data-driven programming）。
...
第四，灵活的模板应用和统一的文档或数据表达。通过 HVML 模板（本质上就是参数化的扩展 JSON 表达式），用于表述树状结构的目标文档内容可由 HVML 生成和更新，这避免了在程序代码中调用特定接口来修改目标文档，HVML 程序只需要关注数据本身的产生和处理即可。这样，就实现了界面和数据的解耦。另外，HVML 对文档和数据的操作提供了一致的操作动作，因此，HVML 操作数据的动作元素，不仅可用于操作数据，也可以可用来操作目标文档。
