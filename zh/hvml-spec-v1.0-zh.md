...
- [附录](#附录)
   + [附.1) 修订记录](#附1-修订记录)
      * [附.1.1) RC1-220209](#附11-rc1-220209)
         - [附.1.1.1) 上下文变量的调整](#附111-上下文变量的调整)
         - [附.1.1.2) `init` 标签的增强](#附112-init-标签的增强)
         - [附.1.1.3) 针对数值执行器的附加说明](#附113-针对数值执行器的附加说明)
         - [附.1.1.4) `observe` 标签的增强](#附114-observe-标签的增强)
         - [附.1.1.5) 骨架元素的增强](#附115-骨架元素的增强)
         - [附.1.1.6) 属性值操作符的增强](#附116-属性值操作符的增强)
      * [附.1.2) 其他](#附12-其他)
   + [附.2) 待定内容](#附2-待定内容)
   + [附.3) 贡献者榜单](#附3-贡献者榜单)
   + [附.4) 商标声明](#附4-商标声明)
...
- `$^`：指父操作的输入数据（input data），初始就是对介词属性 `on` 的属性值求值后的数据；迭代执行器可能会修改这个数据。
- `$&`：指父操作的辅助数据（auxiliary data），初始就是对介词属性 `with` 的属性值求值后的数据；迭代执行器可能会修改这个数据。
...
1. 参数的描述用冒号分隔成两部分；冒号前的部分描述参数的类型和可选的参数名，冒号后的部分描述用途，如：
   - 必传参数：`<number seconds: seconds since epoch>`。
...
1. 返回值类型，在方法后添加冒号描述，可能返回多种类型时，使用 `|` 符号。
...
$EJSON.numberify(<any>, ['number | longint | ulongint | longdouble': the number subtype to return]): number | longint | ulongint | longdouble
```
...
当我们在 `init` 标签中不指定 `as` 或者 `at` 属性时，则初始化或覆盖父栈帧中用户自定义的上下文变量 `$!`。
