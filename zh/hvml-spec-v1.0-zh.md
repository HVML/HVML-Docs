...
1. 数据驱动。一方面，HVML 提供了通过操控数据来实现功能的方法。比如，我们使用更新动作操控定时器数组中的某个字段就可以开启或者关闭一个定时器，而无需调用对应的接口。另一方面，HVML 语言致力于通过统一的数据表达方式将系统中的不同模块串接起来，而不是通过复杂的模块间接口调用来实现。这两个手段可有效地避免传统编程语言存在的接口爆炸问题。为了实现以上目标，HVML 在 JSON 这一广泛使用的抽象数据表达方式之上，提供了扩展的数据类型以及灵活的表达式处理能力。
1. 事件驱动是 HVML 语言的固有特性，HVML 语言提供了观察数据、事件甚至观察一个表达式的值发生变化的机制，开发者无需关心底层的实现机制便可以轻松实现其他编程语言中难以管理的并发或异步编程。
