...
- [2) HVML 详解](#2-hvml-详解)
   ...
   + [2.3) 动作标签](#23-动作标签)
      ...
      * [2.3.15) `call` 和 `return` 标签](#2315-call-和-return-标签)
      * [2.3.16) `catch` 标签](#2316-catch-标签)
      * [2.3.17) `bind` 标签](#2317-bind-标签)
   + [2.4) 执行器](#23-执行器)
      * [2.4.1) 内建执行器](#241-内建执行器)
         - [2.4.1.1) `KEY` 执行器](#2411-key-执行器)
         - [2.4.1.2) `RANGE` 执行器](#2412-range-执行器)
         - [2.4.1.3) `FILTER` 执行器](#2413-filter-执行器)
         - [2.4.1.4) 用于字符串的内建执行器](#2414-用于字符串的内建执行器)
         - [2.4.1.5) 用于数值的内建执行器](#2415-用于数值的内建执行器)
         - [2.4.1.6) `SQL` 执行器](#2416-sql-执行器)
         - [2.4.1.7) `TRAVEL` 执行器](#2417-travel-执行器)
         - [2.4.1.8) 内建执行器的使用](#2418-内建执行器的使用)
      * [2.4.2) 外部执行器](#242-外部执行器)
         - [2.4.2.1) 外部选择器](#2421-外部选择器)
         - [2.4.2.2) 外部迭代器](#2422-外部迭代器)
         - [2.4.2.3) 外部归约器](#2424-外部归约器)
         - [2.4.2.4) 外部函数](#2424-外部函数)
      * [2.4.3) 执行器规则表达式的处理](#243-执行器规则表达式的处理)
   + [2.4) 响应式处理](#24-响应式处理)
- [3) HVML 语法](#3-hvml-语法)
...
<hvml target="html">
    ...
    <body>
        ...
        <observe on=".avatar" for="click">
            <update on="#$?.attr.id" by="FUNC: on_avatar_clicked" />
        </observe>
    </body>
</hvml>
```
...
上述特殊数据类型属于内部数据类型，仅在运行时有效，代码可通过变量和表达式访问。
...
在 HVML 中，我们可以在可变数据上执行执行 `update`、 `erase`、 `clear` 等操作，这些操作本质上修改的是其中的数据项。
...
`$TIMERS`：用于全局的定时器，具有固定的格式，默认为空数组。可使用 `update` 等元素修改它的值，如：
...
在上面的 HVML 代码中，我们在头部使用 `update` 标签设置了 `$T.map`，该变量的内容来自包含有 `$SYSTEM.locale` 的一个 URL。注意其中的 `$SYSTEM.locale` 是一个 JSON 求值表达式，会返回当前系统的语言地区标识符（如 `zh_CN`），HVML 解释器求值并替代后的最终 URL 为：`https://foo.bar/messages/zh_CN`。从该 URL 获得的文件内容一般为：
...
1. 每个元素使用一个字典数据来表述，用 `hvml:_tag` 键值对来描述元素的标签，用 `attr` 键值对来描述元素的属性，用 `content` 键值对来描述该元素的子元素或者内容。
1. 元素的所有属性构成了一个字典数据。
1. 元素的内容：
   - 是字符串时，表示文本内容。
   - 是数组时，所有含有 `hvml:_tag` 键值对的对象，被视作子元素。
   - 其他所有类型的数据，被视作内容。
...
通过动作标签，HVML 可完成对文档或数据的插入、删除、修改等操作，以及通过观察数据的变化而动态调整 DOM 树的行为。我们将在本文档 [2.3) 动作标签](#23-动作标签) 中详细讲述这些动作标签。
...
一个 JSON 求值表达式的返回值类型要么是一个字符串，要么是实际数据，这取决于使用这个表达式的位置。比如在一般的属性（不是 `on` 也不是 `with` 属性）时，将求值为字符串，而在 `archetype` 等模板中，若包含有 `raw` 属性，则所有 `$` 将被忽略。当 JSON 求值表达式被求值为字符串时，非字符串类型的返回值将使用 EJSON 表达，比如空值的字符串表达为 `null`，真值的字符串表达为 `true`，而对象（或字典）的字符串表达就是该对象的 JSON 表述本身。
...
1. `from` 属性指定修改操作源数据的外部数据源，如 URI；此时，使用 `with` 属性指定请求参数。
...
注意，当我们使用 `id` 作为键名时，该键名对应的值，在集合中将保持唯一。

我们使用如下的 `update` 标签以 `$new_users` 作为源数据，使用 `unite` 动作：

```html
    <update on="$users" to="unite" from="http://foo.bar.com/new_users" />
```
