...
1. HVML 可混合使用 HTML、XML 等目标标记语言的标签，使用非 HVML 的标签用于定义一个外部元素。HVML 赋予所有的外部元素一个统一的操作：克隆该元素的属性和内容到目标文档并隐式调整 HVML 程序的上下文数据。
1. HVML 的标签名称、属性名称、变量名称和规则关键词是区分大小写的，这主要是为了和 XML 相关规范保持一致。
1. HVML 的元素支持如下几类属性：
   - 通用属性。目前只有 `id` 一个通用属性；该属性用来指定元素的标识符。在 HVML 中，元素的标识符主要用于定位元素或者执行上下文的位置。在外部元素中使用该属性时，则该属性及其属性值将被克隆到目标文档中。
...
`$global`、 `$users` 这种变量称为命名变量（named variables），又分为静态（static）命名变量和临时（temporary）命名变量；`$?` 这类使用特殊字符的变量称为上下文变量（context variables），根据 HVML 解释器的运行上下文（执行栈）确定其值。

在 HVML 中，上下文变量和静态变量的管理机制不同：

1. 上下文变量存在于 HVML 程序的执行栈上，是临时的，随着相应栈帧的弹出而自动销毁。
1. 静态命名变量是静态的，全局存在的，和 vDOM 树中的某个元素节点关联。除非在 `init` 动作中使用 `undefined` 值重置一个静态命名变量，否则该变量会一直存在。
1. 临时命名变量本质上就是一个特殊的上下文变量。在根据命名变量的名称查找对应的数据时，优先在当前执行栈中查找，然后再在当前 vDOM 位置的父元素以及祖先元素中查找。

HVML 定义的上下文变量罗列如下：

- `$?`：指父操作的结果数据（result data）。
- `$<`：指父操作的输入数据（input data），初始就是对介词属性 `on` 或者 `with` 属性值求值后的数据；迭代时可能会修改这个数据。
...
- `$#<ANCHOR><SYMB>`，如 `$#theAnchor<`、 `$#theAnchor?` 等：指从当前栈帧向上回溯并使用 `<ANCHOR>` 匹配对应元素的 `id` 属性值，从而定位到执行栈上某个栈帧的上下文数据。
...
- 当 `at` 属性指定的祖先元素或祖先栈帧名称不符合要求或者不存在时，将分别抛出 `BadName` 异常或 `EntityNotFound` 异常。
...
- 使用 `as` 属性时，默认将初始化或覆盖父栈帧中用户自定义的上下文变量 `$!` 中指定的键值对，但我们可以使用 `to` 属性通过元素标志符来指定目标栈帧。
...
当我们基于名称查找命名变量时，若没有使用指定位置的上下文变量访问方法，则按如下规则处理：
...
- `$@`：`observe` 的 `in` 属性定义的目标文档位置，或者 `observe` 继承自其父栈帧的目标文档位置。
...
##### RC3.3) 可使用元素的锚点名称定位祖先栈帧

可使用元素的锚点名称定位祖先栈帧，从而可在 JSONEE 中指定祖先栈帧中的上下文变量。
