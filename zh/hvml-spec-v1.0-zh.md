...
1. 当 `with` 属性的属性值是数值型数据，且修改动作为 `displace` 时：
   ...
   - `~=`：对指定的属性值做数值化处理，然后圆整为最接近的整数值替代最终数据（相当于浮点数的 `round` 操作）。
   - `^=`：对指定的属性值做数值化处理，然后取不超过该数值的最大整数值替代最终数据（相当于浮点数的 `floor` 操作）。
   - `$=`：对指定的属性值做数值化处理，然后取不小于该数值的最小整数值替代最终数据（相当于浮点数的 `ceil` 操作）。
