...
2) 对当前 JSONEE 求值，形成当前求值结果，然后，
   ...
   - 若其后是 `&&`，则首先对当前求值结果做布尔化处理，如果是 `true`，则将下个 JSONEE（若有）调整为当前 JSONEE 并跳转到第二步；否则忽略下个 JSONEE（若有）的求值，将当前求值结果设置为 `false` 并跳转到第二步。若没有下个 JSONEE，则跳转到第三步。
   - 若其后是 `||`，则首先对当前求值结果做布尔化处理，如果是 `false`，则将下个 JSONEE（若有）调整为当前 JSONEE 并跳转到第二步，否则忽略下个 JSONEE（若有）的求值 ，将当前求值结果设置为 `true` 并跳转到第二步。若没有下个 JSONEE，则跳转到第三步。
