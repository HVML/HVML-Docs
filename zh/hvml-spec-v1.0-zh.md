...
- `$?`：指父操作的结果数据。
- `$^`：指父操作的操作数据，即父操作中对介词属性 `on` 的属性值求值后的数据。
- `$&`：指父操作的使用数据，即父操作中对介词属性 `with` 的属性值求值后的数据。
...
1. 迭代的初始数据来自 `on` 属性，而每迭代一次，`on` 属性的值将被这次迭代的结果替代。
...
- `$^`：指当前操作数据，即对介词属性 `on` 的属性值求值后的数据。

新增如下上下文变量：

- `$&`：指当前使用数据，即对介词属性 `with` 的属性值求值后的数据。
- `$!`：用户自定义数据，用于定义临时数据。
