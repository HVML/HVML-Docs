...
- 成员运算符：`in`、`not in`；用于检测指定的键名是否存在于对象中，或者指定的值是否在数组、元组中，或者指定的字符是否在字符串中，或者指定的值是否在字节序列中。
...
和运算符表达式相配合，解释器应实现类型构造方法：`$DATA.longint()`、`$DATA.ulongint()`、`$DATA.bigint()`、`$DATA.double()`、`$DATA.longdouble()`，用于构造指定类型的数值。
