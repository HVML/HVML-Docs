...
- [附录](#附录)
   + [附.1) 修订记录](#附1-修订记录)
      * [附.1.1) RC2-220301](#附11-rc2-220301)
         - [附.1.1.1) 用户自定义临时变量的初始化和重置方法](#附111-用户自定义临时变量的初始化和重置方法)
         - [附.1.1.2) 调整动态对象方法的描述语法](#附112-调整动态对象方法的描述语法)
      * [附.1.2) RC1-220209](#附12-rc1-220209)
         - [附.1.2.1) 上下文变量的调整](#附121-上下文变量的调整)
         - [附.1.2.2) `init` 标签的增强](#附122-init-标签的增强)
         - [附.1.2.3) 针对数值执行器的附加说明](#附123-针对数值执行器的附加说明)
         - [附.1.2.4) `observe` 标签的增强](#附124-observe-标签的增强)
         - [附.1.2.5) 骨架元素的增强](#附125-骨架元素的增强)
         - [附.1.2.6) 属性值操作符的增强](#附126-属性值操作符的增强)
      * [附.1.3) 其他](#附13-其他)
   + [附.2) 待定内容](#附2-待定内容)
   + [附.3) 贡献者榜单](#附3-贡献者榜单)
   + [附.4) 商标声明](#附4-商标声明)
...
- `locally`：在 `init` 动作元素中，用于操作用户自定义上下文变量。
