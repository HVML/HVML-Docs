...
- 所有动作元素的 `on` 和 `with` 属性，若赋值操作符（=）被忽略且使用无引号属性值语法，或者使用其他语法情形下，以 `[`、 `{`、 `$` 打头时，将被视作一个参数化数据处理；否则按字符串处理。
- `iterate` 元素的 `onlyif` 和 `while` 属性，若赋值操作符（=）被忽略且使用无引号属性值语法，或者使用其他语法情形下，以 `[`、 `{`、 `$` 打头时，将被视作一个参数化数据处理；否则按字符串处理。
