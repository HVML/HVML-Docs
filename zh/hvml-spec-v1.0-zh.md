...
为方便执行常见的算术或逻辑运算而引入运算符表达式。HVML 的运算符表达式可出现任意的 HVML 求值表达式中。使用运算符表达式有如下好处：

- 方便通过常见运算符执行算术、逻辑、字符串、位操作等。
...
其中不带下划线后缀的变量是线性容器，可通过索引访问任意栈帧中的对应上下文变量；带有下划线后缀的变量则特指上个栈帧中的上下文变量。 如 `_pos[0]` 表示当前栈帧定义的位置上下文变量。

定义运算符表达式的方式大部分情况下使用 `(...)` 包围即可。如：
