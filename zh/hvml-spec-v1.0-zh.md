...
- `change:modified`：表示容器中某个或多个（直接子）数据项的值发生了变化：
   - 若容器为线性容器，则该事件的负载为被修改的数据项的索引值构成的元组。
   - 若容器为对象，则该事件的负载为被修改的属性之键名构成的元组。
- `change:inflated`：表示容器中新增了一条或多条子数据项：
   - 若容器为线性容器，则该事件的负载为新增的数据项之索引值构成的元组；注意索引值是该数据项被添加到线性容器后的索引值。
   - 若容器为对象，则该事件的负载为新增属性之键名构成的元组。
- `change:deflated`：表示容器中移除了一条或多条子数据项：
   - 若容器为线性容器，则该事件的负载被移除的数据项之索引值构成的元组；注意索引值是该数据项被移除前在线性容器中的索引值。
   - 若容器为对象，则该事件的负载为被移除属性之键名构成的元组。
