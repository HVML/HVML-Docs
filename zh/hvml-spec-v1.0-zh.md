...
1. HVML 的标签定义一个动作元素，每个动作元素执行一个或者一组操作，所以这些标签使用英文中的动词。而 HTML 标准通常使用名词或其简写作为标签名称，如 HTML 的常见标签 `p` 是 paragraph（段落）的简写。
1. HVML 可混合使用 HTML 的标签，使用 HTML 的标签可定义一个外部元素，HVML 赋予所有的外部元素一个统一的操作：克隆该元素到目标文档并隐式调整目标文档操作位置到该元素所在位置。
1. HVML 的标签名称、属性名称、变量名称和规则关键词是区分大小写的，这主要是为了和 XML 相关规范保持一致。

现在对照上面的例子，介绍 HVML 的一些特点。本文档其他章节也会引用这个例子的片段。
