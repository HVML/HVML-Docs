...
1. `元素（element）`。指文档对象模型中，使用某个标签（tag）定义的元素节点；一个元素可包含一个或多个属性（attribute）以及属性值，还可以包含内容（content）；一个元素可包含文本内容、数据内容或者使用标签定义的单个或多个子元素。在本文档的某些上下文中，为避免可能的混淆，会将目标 XML/HTML 文档定义的元素称作 `文档元素`；将 `HVML` 程序定义的元素称作 `动作元素`。
...
1. `码点（code point）`。指一个表述为 `U+` 和四到六个 ASCII 大写十六进制数字形式的 Unicode 码点，范围在 U+0000 到 U+10FFFF（含）。有时候，我们会在码点之后包含码点的名称以及包含在小括号中的该码点的渲染形式，且高亮或加粗显示该码点的渲染形式。对无法渲染的码点，本文档会给出其码点名称。有关 Unicode 字符的更多术语解释如下：
   ...
   - `替代符（surrogate）`是范围在 U+D800 到 U+DFFF（含）的码点；此范围的码点仅用于 UTF-16 编码。
...
- 在求值表达式中，可使用 `()` 定义一个支持运算符的表达式，如算术表达式、逻辑表达式等。如：`$DATA.count([($0< + 2), 2, true, (SYS.time > 1000), "$user.id"])`。

参数化数据通常用于指定可接受数据的介词属性（如 `on` 和 `with`）的属性值以及动作元素的数据内容，而参数化字符串通常用于指定属性值以及文档片段模板。有关属性值的指定语法，见本文档 [3.1.2.4) 动作元素属性](#3124-动作元素属性) 一节。
...
为方便执行常见的算术或逻辑运算而引入运算符表达式。HVML 的运算符表达式可出现在任意的 HVML 求值表达式中，亦可在运算符表达式中包含 HVML 求值表达式。

使用运算符表达式有如下好处：

- 方便通过常见运算符执行算术、比较、逻辑、字符串、位操作等。
- 在运算符表达式中，除上下文变量之外，可免去变量名之前的 `$` 符号。
- 对上下文变量，亦可使用如下别名（均为保留关键词）来指代上级栈帧中的上下文变量，从而避免使用 `$` 符号：
   - `@`：`_pos` 或 `_pos_`。
   - `?`：`_res` 或 `_res_`。
   - `!`：`_usr` 或 `_usr_`。
   - `^`：`_cnt` 或 `_cnt_`。
...
其中不带下划线后缀的变量可视作是一个线性容器，通过索引访问前置栈帧中的对应上下文变量；带有下划线后缀的变量则特指上一个栈帧中的上下文变量。如 `_pos[0]` 表示当前栈帧定义的位置上下文变量；而 `_pos_` 表示上一个栈帧中的位置上下文变量。

要使用运算符表达式，大部分情况下使用 `(...)` 包围即可。如：

```hvml
    <test with (DATETIME.localtime['year'] > 100) >
        ...
    </test>
```

出现在函数调用参数列表中时，由于已经有了小括号，故而可省略小括号：
...
但当出现在参数化字符串中时，需要使用 `$(...)` 包围，如：

```hvml
"HVML age: $(DATETIME.localtime['year'] + 1900 - 2021)"
```

解释器应实现如下运算符：

- 算术单目运算符：`+`、`-`；分别执行保持原值（但产生新数）、取相反数的操作。对于非数值类型，首先执行数值化。
- 算术双目运算符：`+`、`-`、`*`、`/`、`%`、`//`、`**`。要求操作数（operand）必须是数值类型。注意可能的类型提升。
- 比较运算符：`==`、`!=`、`>`、`>=`、`<`、`<=`。对非数值类变体，执行数值化后再进行比较。
- 逻辑运算符：`not`、`and`、`or`。对于非布尔类变体，执行布尔化后再进行相应的运算。
- 成员运算符：`in`、`not in`；用于检测指定的键名是否存在于对象中，或者指定的值是否在数组、元组中，或者指定的子串是否在字符串中，或者指定的子序列是否在字节序列中。
- 位运算符：`&`、`|`、`~`、`^`、`<<`、`>>`。针对 `longint`、`ulongint` 和 `bigint` 类变体执行对应的位运算及移位运算。
- 串接运算符：`+`。可用于字符串、字节序列、数组等。
- 合并运算符：`+`。当左操作数为对象时，右操作数必须为对象；当左操作数为集合时，右操作数必须为数组或元组。
- 条件运算符：`条件表达式 ? 表达式1 : 表达式2`；当 `条件表达式` 的求值结果为真时取 `表达式1` 的结果，否则取 `表达式2` 的结果。
- 逗号运算符：`表达式1, [表达式2, ...]`。依次对各表达式求值，各表达式的求值结果构成一个元组作为结果。其语法规则类似 Python 中定义元组的规则。
- 赋值运算符：`=`。结果为运算符右侧操作数。
- 运算并赋值运算符：`++`、`--`、`+=`、`-=`、`*=`、`/=`、`%=`、`//=`、`**/`、`&=`、`|=`、`~=`、`^=`、`<<=`、`>>=`。结果为运算符右侧操作数。
...
注意：默认情况下，赋值运算符定义的变量始终为临时变量，若在变量名之前使用 `static` 限定词，则定义静态变量。

和运算符表达式相配合，解释器应实现数值类数据的构造方法，如 `$DATA.longint()`、`$DATA.ulongint()`、`$DATA.bigint()`、`$DATA.double()`、`$DATA.longdouble()`，用于构造指定类型的数值。

另外，在静默求值情况下，当运算符表达式中的某个运算遭遇异常时，其结果按 `null` 处理。
...
- 操作系统相关：
   ...
   - `ProtocolViolation`：未遵从协议。
   - `TLSFailure`：TLS 相关错误。该异常应定义额外信息以便应用程序可以获得具体的请求失败信息，如 OpenSSL 错误信息。
...
- `on`：在 `choose`、`iterate` 等操作数据的标签中，用于定义执行动作所依赖的数据、元素或元素汇集，或者用于显示指定一个新的执行结果数据。
- `from`：在 `init`、 `update`、 `define`、 `load` 等支持外部资源的元素中，用于定义执行动作所依赖的外部资源，其属性值通常是一个 URL。
- `via`：和 `from` 属性配合适用，用于指定请求方法（如 `GET`、 `POST`、 `DELETE` 等）以及请求头部信息。若附加使用 `RAW-HEADERS` 关键词，表明请求参数为自定义的 HTTP 请求头部。
...
- `wholly`：在 `update` 元素中，用于定义当源数据为一个线性容器时，如何看待源数据。使用该副词时将源数据看成一个整体参与更新操作，否则取出容器中的成员参与更新操作。
...
- 我们可以使用 `prop.selectedIndex` 或 `prop.style.width`、`prop.style[width]` 这样的写法来引用一个文档元素的动态属性（property）或渲染用样式属性（style property）。这两种属性由渲染器维护，统称为动态属性。当引用一个未定义的动态属性时，按其属性值为 `undefined` 对待。

通常，我们通过 `update` 动作元素修改文档元素的静态属性（attribute）和内容（content）；而对动态属性（property），比如 `input` 框中输入的内容，则需要使用 `request` 动作元素来获取或者设置。

文档元素的动态属性通常用于反映元素在界面上的展现状态或者交互状态，比如 `input` 元素对应的输入框中，用户在输入框中输入的内容、当前插入符的位置、当前选中的范围等均属于动态属性。文档元素的动态属性由渲染器维护，加载时，将从文档元素的静态属性中初始化动态属性。在 HVML 程序中，应使用 `request` 动作元素来获取或设置文档元素的动态属性。我们通过 `update` 动作元素修改文档元素的静态属性，相当于修改 eDOM，也会将此更改同步给渲染器。
...
我们引入 `hvml` 和 `hvml+run` 两种 URI 方案（Schema）用于 HVML 应用框架。

##### 2.1.19.1) `hvml` 方案

该方案主要用于 HVML 渲染器，有两种用途：

1) 定义 HVML 渲染器中的页面

和 `http` 方案类似，完整的 `hvml` 方案包括主机名、应用名、行者名、页面组名称和页面名称以及查询（query）组件，如：
...
- 行者名称：
   ...
   - 使用 `_http`、 `_https`、 `_ftp`、 `_file` 等保留名称指代可重定向的 URL 方案。
- 页面组名称：始终使用 `-`。
...
##### 2.1.19.2) `hvml+run` 方案

该方案主要用于定义属于特定行者的协程或者通道，和 `ftp` 方案类似，完整的 `hvml+run` 方案包括主机名、应用名、行者名、资源类型（表示协程的 `CRTN` 或者表示通道的 `CHAN`）及资源标志符，如协程令牌或通道名称，如：

`hvml+run://localhost/appName/myRunner/CRTN/7`

在 HVML 程序中，我们可以通过 `request` 等动作元素和当前应用的其他行者交互。方便起见，我们无需指定方案名称和主机名称，并可使用 `-` 指代当前应用，这样就可以使用如下简写方式引用当前主机、当前应用中属于指定行者的协程或通道，如：
...
在 `hvml+run` 方案中，我们保留如下的特殊协程令牌（相当于别名）：
...
4) 使用如下后缀来明确表示数值（number）的类型：
   ...
   - `N` 后缀表示大整数，如 `1234567890N`；可使用 `0x` 或 `0` 前缀表示大整数的十六进制或八进制表达，但不可忽略其后的 `N` 后缀。
...
- `template`：定义 HVML 程序初始装载的目标文档文件（模板），该文件将被装载作为初始的 eDOM。使用预先准备好的 HTML 模板文件，可避免使用 HVML 生成 HTML 文档，从而提高 HVML 程序的首轮执行速度。另一方面，使用模板之后，我们可以在 HVML 程序中仅保留基于模板的内容变更代码，从而保持程序逻辑简洁、清晰。注意当 `target` 为 `void` 时，将忽略该属性。
...
- 若初始化静态变量，当我们使用下划线（\_）打头的预定义名称 `_parent/_last`、`_grandparent/_nexttolast`、 `_root/_topmost` 时，将分别在父元素、祖父元素或根元素上定义变量。
- 若初始化临时变量，当我们使用下划线（\_）打头的预定义名称 `_last/_parent`、`_nexttolast/_grandparent`、 `_topmost/_root` 时，则分别在上一个栈帧、上上一个栈帧和最顶栈帧上定义临时变量。
...
在使用 `with` 属性指定 `HTTP` 等请求的参数时，默认使用 `application/x-www-form-urlencoded` 类型。若需要使用自定义的 HTTP 请求头部，可在 `via` 属性值中指定 `RAW-HEADERS` 关键词（如 `POST RAW-HEADERS`），并使用 `with` 指定自定义的 HTTP 请求头部内容。
...
```hvml
    ...
    <init as="locales" from="http://foo.bar.com/locales" via="POST RAW-HEADERS" >
        '''
...
1. `to` 属性指定具体的修改动作（action），可取如下值之一：
   ...
   - `append`：表示在目标数据或目标位置中执行追加操作；最终数据必须是数组或者目标文档位置。当源数据为线性容器时，针对源数据可以有两种处理方法：使用 `wholly` 副词属性时，将源数据视作单个整体执行追加操作；若不使用 `wholly` 副词属性，则依次取出容器中的成员执行追加操作。
   - `prepend`：表示在目标数据或目标位置中执行前置操作；最终数据必须是数组或者目标文档位置。当源数据为线性容器时，针对源数据可以有两种处理方法：使用 `wholly` 副词属性时，将源数据视作单个整体执行前置操作；若不使用 `wholly` 副词属性，则依次取出线性容器中的成员执行前置操作。
   - `insertBefore`：表示在目标数据或者目标位置之前插入一个数据；最终数据必须是数组或者目标文档位置。当源数据为线性容器时，针对源数据可以有两种处理方法：使用 `wholly` 副词属性时，将源数据视作单个整体执行前插入操作；若不使用 `wholly` 副词属性，则依次取出线性容器中的成员执行前插入操作。
   - `insertAfter`：表示在目标数据或者目标位置之后插入一个数据；最终数据必须是数组或者目标文档位置。当源数据为线性容器时，针对源数据可以有两种处理方法：使用 `wholly` 副词属性时，将源数据视作单个整体执行后插入操作；若不使用 `wholly` 副词属性，则依次取出线性容器中的成员执行后插入操作。
   - `add`：当最终数据是数组或者目标文档位置时，等同于 `append` 动作；当最终数据是为集合时，则将源数据添加到指定的集合中。第二种情况下，当源数据为线性容器时，针对源数据可以有两种处理方法：使用 `wholly` 副词属性时，将源数据视作单个整体执行添加操作；若不使用 `wholly` 副词属性，则依次取出线性容器中的成员执行添加操作。
   - `remove`：当最终数据是数组或者目标文档位置时，表示移除目标位置上的数据；当最终数据是集合时，表示从集合中移除一个和源数据匹配的数据项。第二种情况下，当源数据为线性容器时，针对源数据可以有两种处理方法：使用 `wholly` 副词属性时，将源数据视作单个整体执行移除操作；若不使用 `wholly` 副词属性，则依次取出线性容器中的成员执行移除操作。
...
当更新的最终数据是对象、元组、数组、集合，且具有 `individually` 副词属性时，`update` 标签设定的更新动作，将作用于数组中所有的数据项在指定位置上的数据。如，
...
`test` 标签和 `match` 标签配合使用，主要用于实现分支处理。`test` 元素通过 `on` 属性定义执行要匹配的目标数据（该数据也是该元素的结果数据），而 `match` 作为 `test` 元素的子元素，每个 `match` 子元素定义一个匹配分支。
...
对于匹配条件，我们可以在 `match` 元素中使用 `with` 介词属性来定义一个表达式，通过对该表达式求值的结果做布尔化处理来判断是否匹配，也可以使用 `for` 介词属性定义一个基于 `test` 元素执行结果的匹配条件，两者选一，但 `for` 属性具有更高优先级。

使用 `with` 介词属性时，我们可以使用一个求值表达式来确定匹配条件；当求值结果做布尔化处理后，若结果为 `true`，则视作匹配，反之视作不匹配。比如就上述 HVML 代码中的匹配 `zh_CN` 的 `match` 元素，可以如下书写：

```hvml
        <match with="$L.streq('case', 'zh_CN', $?)" exclusively>
            <update on="$@" to="displace" with="$footer_cn" />
        </match>
```
...
若 `match` 元素不指定 `for` 或者 `with` 属性，则视作匹配任何条件。

`match` 动作元素默认继承前置操作（即 `test`）的结果数据作为其结果数据，但可使用 `on` 属性值覆盖这一结果数据。如下所示：

```hvml
    <test on $aNumber>
        <match with $L.gt($aNumber, 0) exclusively>
            <inherit>
                $STREAM.stdout.writelines("$? is greater that 0")
            </inherit>
        </match>

        <match with $L.lt($aNumber, 0) exclusively>
            <inherit>
                $STREAM.stdout.writelines("$? is less than 0")
            </inherit>
        </match>

        <match on "$? is equal to 0" >
            <inherit>
                $STREAM.stdout.writelines($?)
            </inherit>
        </match>
...
以上的代码，当 `with` 属性的值为 `true` 时，继续执行该元素的操作子树，但忽略 `differ` 子元素定义的操作子树；如果属性值求值后的结果为 `false`，则仅处理 `differ` 子元素定义的操作子树。这种场景下，`differ` 元素是可选的。`differ` 元素将忽略 `on` 和 `in` 之外的任何介词属性和副词属性。和 `inherit` 类似，该元素会默认继承前置操作的上下文变量。但若定义有 `on` 属性，则使用 `on` 属性的求值结果作为其结果数据；若定义有数据内容，则使用数据内容覆盖对应栈帧的 `$^` 上下文变量。

> 增强计划：可考虑为 `differ` 元素增加 `but` 介词属性，用于实现传统编程语言的 `else if` 功能。

当某个 `test` 动作元素同时指定有 `with` 和 `on` 属性时，仍然按二选一方案处理，但此时 `on` 属性的求值结果可用于覆盖继承自前置操作的结果数据。
...
`define` 元素可使用 `from` 属性从指定的 URL 中装载 HVML 片段。借助此功能，我们可以将具有不同功能的操作组作为公共模块供不同的 HVML 程序使用。另外，当同时使用 `define` 元素的内容和 `from` 属性指定的 HVML 片段定义一个操作组时，该操作组将首先使用内容定义，当正确装载由 `from` 属性定义的 HVML 片段后，使用装载后的 HVML 片段绑定到操作组名称上。
...
- `change:modified`：表示容器中某个或多个（直接子）数据项的值发生了变化：
   - 若容器为线性容器，则该事件的负载为被修改的数据项的索引值构成的数组。
   - 若容器为对象，则该事件的负载为被修改的属性之键名构成的数组。
- `change:inflated`：表示容器中新增了一条或多条子数据项：
   - 若容器为线性容器，则该事件的负载为新增的数据项之索引值构成的数组；注意索引值是该数据项被添加到线性容器后的索引值。
   - 若容器为对象，则该事件的负载为新增属性之键名构成的数组。
- `change:deflated`：表示容器中移除了一条或多条子数据项：
   - 若容器为线性容器，则该事件的负载被移除的数据项之索引值构成的数组；注意索引值是该数据项被移除前在线性容器中的索引值。
   - 若容器为对象，则该事件的负载为被移除属性之键名构成的数组。
...
- `_observedOn`：`on` 属性定义的值；当 `against` 属性生效时，不定义此属性。
- `_observedFor`：`for` 属性定义的值。
- `_observedIn`：`in` 属性定义的值；若存在则为字符串。该值将在事件到来时用于确定上下文变量 `$@` 的值。
- `_observedWith`：`with` 属性的值。
- `_observedContent`：内容的值；无内容时不定义此属性。

当 `observe` 观察的事件到来时，解释器应切换到 `observe` 自身定义的操作组或者其 `with` 属性引用的由 `define` 定义的操作组中执行 HVML 程序；此时，解释器不应该对其属性和内容再次执行求值操作，取而代之，解释器应在其对应操作组的前置栈帧中，基于事件或前述的隐式对象确定如下上下文变量的值：

- `$?`：事件的负载（payload）数据；若被观察的是变量，则就是被观察变量对应的数据。
- `$!`：在用户数据中，预定义三个临时变量，用于表示事件主名称、子名称和事件源，名称分别为 `_eventName`、 `_eventSubName` 和 `_eventSource`，并将前述隐式对象各属性合并到 `$!` 中，使之可作为临时变量供程序访问。
- `$@`：根据隐式对象中的 `_observedIn` 属性确定的目标文档位置；若不存在，则：
...
最后，`observe` 支持 `once` 副词属性，具有该副词属性的观察，将在执行一次后由解释器自动解除。此种情况下，解释器应移除隐式对象。
...
- `on`：指定要调用的操作组。
- `with`：指定传入操作组的实参。当未定义 `with` 属性时，使用 `call` 元素的内容作为 `with` 属性值；若两者皆未定义，则无实参传入操作组；若同时指定 `with` 属性和元素内容，则分别作为被调用操作组的 `$?` 数据和 `$^` 数据。
- `in`：指定文档位置。
...
`inherit` 标签用于定义一个执行继承操作的动作元素，除 `on` 和 `in` 属性之外，该元素不使用其他介词和副词属性。该标签定义的动作元素默认继承其前置栈帧的上下文变量。但是，若定义有 `on` 属性值，则使用该属性值表达式的求值结果覆盖对应栈帧的 `$?` 上下文变量；若定义有数据内容，则使用数据内容覆盖对应栈帧的 `$^` 上下文变量。
...
如果我们使用 HybridOS 中提到的直接执行本地系统命令的扩展方案（lcmd），我们甚至都不需要编写任何代码，而只需要使用 `init`：
...
- 新增大整数（bigint）类型，用于支持高精度整数运算；其字面值使用 `n/N` 后缀，支持使用 `0x` 或 `0` 做为其字面值的十六进制和八进制前缀。
...
##### OR1.1) 调整 `change:*` 事件的负载

- 定义了针对容器的 `change:*` 事件的负载，以方便编程。
...
1. 新增 `ProtocolViolation` 和 `TLSFailure` 两个异常。
...
增强 `hvml` 方案以支持 URL 重定向。

相关章节：

- [2.1.19.1) `hvml` 方案](#21191-hvml-方案)

##### RCg.2) 增强 `hvml` 标签使之支持 `template` 属性

主要修订内容如下：

在 `hvml` 标签中，可使用 `template` 属性定义 HVML 程序初始装载的目标文档模板文件，该文件将被装载作为初始的 eDOM。
...
##### RCf.1) `observe` 中上下文变量的规则调整

主要修订内容如下：

对上下文变量 `$@`，若事件来自渲染器页面上的某个元素，且未通过 `in` 属性显式指定文档位置，则优先定义为产生该事件的元素对应的目标文档位置。
...
##### RCd.1) 调整某些动作元素的属性及执行规则

主要修订内容如下：

1. 和 `in` 属性类似，部分不使用 `on` 属性的动作元素，其执行结果将自动继承前置操作的执行结果。
1. 外部元素的执行结果数据，始终继承自前置操作。
1. 当 `test` 标签中使用 `with` 属性定义测试条件时，默认继承前置操作的结果数据，但当使用 `on` 属性值时，用该属性值的求值结果作为结果数据。
1. `match` 使用 `with` 属性定义匹配表达式，不再使用 `on` 属性，从而可默认继承前置操作的结果数据。
1.  若 `match` 元素不指定 `for` 或者 `with` 属性，则视作匹配任何条件。
1. 和 `inherit` 类似，`differ` 元素会默认继承前置操作的所有上下文变量。
...
##### RCd.2) 调整 `update` 元素并新增 `wholly` 副词属性

在 `update` 元素中，用于定义当源数据为一个线性容器时，如何看待源数据。使用该副词时将源数据看成一个整体参与更新操作，否则取出容器中的成员参与更新操作。
...
1. 使用 `RAW-HEADERS` 来声明请求参数为自定义的 HTTP 请求头部
...
##### RC6.3) 调整 HVML URI 方案

主要修订内容如下：

1. 调整 `hvml+cor` 方案为 `hvml+run` 方案。
1. 增强 `hvml+run` 方案，使之可以用来指定协程或者通道。
