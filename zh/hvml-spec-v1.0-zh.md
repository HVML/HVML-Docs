...
- [2) HVML 详解](#2-hvml-详解)
   + [2.1) 基本原理](#21-基本原理)
      ...
      * [2.1.5) 可变数据和不可变数据](#215-可变数据和不可变数据)
         - [2.1.5.1) 浮点类数据的固有方法](#2151-浮点类数据的固有方法)
         - [2.1.5.2) 整型数据的固有方法](#2152-整型数据的固有方法)
         - [2.1.5.3) 字符串的固有方法](#2153-字符串的固有方法)
         - [2.1.5.4) 字节序列的固有方法](#2154-字节序列的固有方法)
         - [2.1.5.5) 数组的固有方法](#2155-数组的固有方法)
         - [2.1.5.6) 元组的固有方法](#2156-元组的固有方法)
         - [2.1.5.7) 集合的固有方法](#2157-集合的固有方法)
         - [2.1.5.8) 对象的固有方法](#2158-对象的固有方法)
      * [2.1.6) 变量](#216-变量)
...
- 复数（complex），内部用两个 C 语言的 double 类型分别表示复数的实部和虚部。
- 字节序列（bsequence）。
...
- 数值类（number、longint、ulongint、double、long double、complex）。
- 字符串（string）。
- 字节序列（byte sequence）。
- 数组（array）。
- 元组（tuple）。
...
HVML 解释器应在浮点类（number、longdouble、complex）数据上至少提供如下固有方法：

- `$x.sum($y)`：返回 `$x` 和 `$y` 的和。
- `$x.diff($y)`：返回 `$x` 和 `$y` 的差。
- `$x.prod($y)`：返回 `$x` 和 `$y` 的积。
- `$x.quot($y)`：返回 `$x` 和 `$y` 的商。
- `$x.mod($y)`：返回 `$x` 和 `$y` 相除的余数。
- `$x.round($n)`：四舍五入到保留指定的小数点位数（`$n` 默认为 0）。
- `$x.pow($y)`：返回 `$x` 的 `$y` 幂次值。
- `$x.max($y, ...)`：将 `$x` 和 `$y` 相比，返回其中的最大值。
- `$x.min($y, ...)`：将 `$x` 和 `$y` 相比，返回其中的最小值。
- `$x.trunc()`：将 `$x` 截断为整数值并返回。
- `$x.floor()`：将 `$x` 四舍五入到不大于初始值的最大整数值。
- `$x.ceil()`：将 `$x` 四舍五入到不小于初始值的最小整数值。
- `$x.abs()`：返回 `$x` 的绝对值。
- `$x.neg()`：返回 `$x` 的负值。

以上获取器均具有对应的设置器，用以使用结果修改原始值。当原始值发生变化时，可在对应的变量上产生 `change` 事件。

##### 2.1.5.2) 整型数据的固有方法

HVML 解释器应在整型（longint、ulongint）数据上至少提供如下固有方法：
...
HVML 解释器应为字符串至少提供如下固有方法：

- `$string.len()`：返回字符串长度（字符个数）。
- `$string.lower()`：返回 `$string` 对应的小写版本。
- `$string.upper()`：返回 `$string` 对应的大写版本。

##### 2.1.5.4) 字节序列的固有方法

HVML 解释器应为字节序列提供如下固有方法：

- `$bseq.len()`：返回字节序列的长度（字节个数）。

##### 2.1.5.5) 数组的固有方法

HVML 解释器应为数组提供如下固有方法：

- `$array.len()`：返回数组长度（成员个数）。
- `$array.in($x)`：判断数组中是否包含有 `$x`。
- `$array.notin($x)`：判断数组中是否不包含 `$x`。
- `$array.concat($x)`：返回连接 `$array` 和 `$x` 的一个新数组；`$x` 可以是一个数组，也可以是一个元组。
- `$array.concat(! $x )`：将 `$x` 连接到 `$array` 尾部；`$x` 可以是一个数组，也可以是一个元组。
- `$array.repeat($n)`：创建一个重复 `$array` `$n` 次的新数组。
- `$array.repeat(! $n )`：在 `$array` 中重复自己的内容 `$n` 次。
- `$array.min([$cmp_method])`：返回数组中最小的值；`$cmp_method` 指定对比方法，默认为数值。
- `$array.max([$cmp_method])`：返回数组中最大的值；`$cmp_method` 指定对比方法，默认为数值。
- `$array.count($x[, $cmp_method])`：返回数组出现 `$x` 的次数；`$cmp_method` 指定对比方法，默认为数值。
- `$array.index($x[, $cmp_method[, $i[, $j[, $k]]]])`：返回在 `i:j:k` 定义的数组片段中出现 `$x` 的索引值；`$cmp_method` 指定对比方法，默认为数值。
- `$array.append($x)`：在数组尾部追加新成员。
- `$array.prepend($x)`：在数组头部添加新成员。
- `$array.del($i[, $j[, $k]])`：在 `i:j:k` 定义的数组片段中移除成员；`$k` 指定步长。
- `$array.remove($x, $i[, $j[, $k]])`：在 `i:j:k` 定义的数组片段中移除第一个匹配 `$x` 的成员；`$k` 指定步长。
- `$array.set($x, $i[, $j[, $k]])`：在 `i:j:k` 定义的数组片段中设置所有成员的值为 `$x`。
- `$array.clear()`：清空数组。
- `$array.reverse()`：倒置数组。
- `$array.insertBefore($i, $x)`：在 `$i` 位置之前插入 `$x`。
- `$array.insertAfter($i, $x)`：在 `$i` 位置之后插入 `$x`。

##### 2.1.5.6) 元组的固有方法

HVML 解释器应为元组提供如下固有方法：

- `$tuple.len()`：返回元组长度（成员个数）。
- `$tuple.in($x)`：判断元组中是否包含有 `$x`。
- `$tuple.notin($x)`：判断元组中是否不包含 `$x`。
- `$tuple.concat($x)`：返回连接 `$tuple` 和 `$x` 的一个新元组；`$x` 可以是一个元组，也可以是一个数组。
- `$tuple.repeat($n)`：创建一个重复 `$tuple` 中成员 `$n` 次的新元组。
- `$tuple.min([$cmp_method])`：返回元组中最小的值；`$cmp_method` 指定对比方法，默认为数值。
- `$tuple.max([$cmp_method])`：返回元组中最大的值；`$cmp_method` 指定对比方法，默认为数值。
- `$tuple.count($x[, $cmp_method])`：返回元组出现 `$x` 的次数；`$cmp_method` 指定对比方法，默认为数值。
- `$tuple.index($x[, $cmp_method[, $i[, $j[, $k]]]])`：返回在 `i:j:k` 定义的元组片段中出现 `$x` 的索引值；`$cmp_method` 指定对比方法，默认为数值。
- `$tuple.set($x, $i[, $j[, $k]])`：在 `i:j:k` 定义的元组片段中设置所有成员的值为 `$x`。
- `$tuple.reverse()`：倒置元组。

##### 2.1.5.7) 集合的固有方法

HVML 解释器应为集合提供如下固有方法：

- `$set.len()`：返回集合长度（属性个数）。
- `$set.in($x)`：判断集合中是否包含 `$x`。
- `$set.notin($x)`：判断集合中是否不包含 `$x`。
- `$set.add($x)`：向集合中添加一个成员。
- `$set.remove($x)`：从集合中移除其值等于 `$x` 的成员并返回这个值。
- `$set.discard($x)`：从集合中废弃其值等于 `$x` 的成员。
- `$set.pop($x)`：从集合中移除一个成员。
- `$set.clear()`：从集合中清除所有的成员。
- `$set.isdistjoint($other)`：判断 `$other` 是否和 `$set` 不相交（没有任何共同成员）。
- `$set.issub($other)`：判断 `$other` 是否是 `$set` 的子集。
- `$set.issuper($other)`：判断 `$other` 是否是 `$set` 的超集。
- `$set.union($other)`：创建新集合并合并 `$other` 和 `$set`，如遇重复值，则使用 `$other` 中的值覆盖。
- `$set.union(! $other )`：合并 `$other` 集合到 `$set` 集合中，如遇重复值，则使用 `$other` 中的值覆盖。
- `$set.substract($other)`：创建新集合使之为 `$set` 中减去 `$other` 中相同的成员。
- `$set.substract(! $other )`：从 `$set` 中减去 `$other` 中相同的成员。
- `$set.intersect($other)`：用 `$set` 和 `$other` 的交集创建一个新集合。
- `$set.intersect(! $other )`：用 `$set` 和 `$other` 的交集覆盖 `$set`。
- `$set.xor($other)`：用 `$set` 和 `$other` 的亦或操作结果创建一个新集合。
- `$set.xor(! $other )`：用 `$set` 和 `$other` 的亦或操作结果覆盖 `$set`。

##### 2.1.5.8) 对象的固有方法

HVML 解释器应为对象提供如下固有方法：

- `$obj.len()`：返回对象长度（属性个数）。
- `$obj.keys()`：返回对象中所有属性键名构成的数组。
- `$obj.values()`：返回对象中所有属性值构成的数组。
- `$obj.del($key)`：移除对象中键名 `$key` 对应的属性。
- `$obj.in($key)`：判断对象中是否包含有键名 `$key` 对应的属性。
- `$obj.notin($key)`：判断对象中是否不包含键名 `$key` 对应的属性。
- `$obj.clear()`：从对象中清除所有的属性。
- `$obj.get($key, [$def])`：返回对象中键名 `$key` 对应的属性值；如果未给定 `$def` 且 `$key` 不在该对象，则产生 `NoSuchKey` 异常。
- `$obj.pairs()`：返回对象中所有的属性（键值对），每个属性使用一个元组表示。
- `$obj.pop($key, [$def])`：如果 `$key` 在对象中，则移除对应的属性并返回属性值，否则返回 `$def`；如果未给定 `$def` 且 `$key` 不在该对象，则产生 `NoSuchKey` 异常。
- `$obj.setdef($key, [$def])`：如果 `$key` 在对象中，则返回其属性值，否则使用 `$key` 为键名、`$def` 为属性值添加一个新的属性；如果未给定 `$def`，则取 `null`。
- `$obj.merge($other)`：创建新对象并合并 `$other` 和 `$obj`，如遇重复键名，则使用 `$other` 的键值覆盖。
- `$obj.merge(! $other )`：合并 `$other` 对象到 `$obj` 对象中，如遇重复键名，则使用 `$other` 的键值覆盖。
