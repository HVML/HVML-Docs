...
- `onto`：指定用于渲染目标文档的渲染器页面标识符，使用 `[<page_type>:]<page_name>[@[<workspace_name>/]<group_name>]` 这样的形式，用于指定页面名称和所在的页面组。其中 `<page_type>` 指定页面的类型，可使用 `plainwin`（默认） 或 `widget`，分别表示创建一个普通窗口或者小构件作为页面；`workspace_name` 和 `group_name` 分别表示页面所在的工作区名称以及页面组名称，`page_name` 是页面在指定页面组中的唯一性名称。

指定页面名称时，我们可以使用如下保留名称（保留名称通常以下划线打头）指代特定的页面：

- `_null`：表示空页面。此时，新创建的协程所生成的文档内容及其变更信息将不会同步到渲染器。当不指定 `onto` 属性时，视作创建空页面。使用该保留名称时，不需要指定页面组和页面类型。
- `_inherit`：若新创建的子协程和当前协程属于同一行者，则表示该协程将继承父协程的文档内容，且使用相同的渲染器页面。此时，子协程和父协程可同时更新该文档，且其更新将同时反映到当前页面上。使用该保留名称时，不需要指定页面组和页面类型。
- `_self`：表示当前页面。在当前页面中渲染新的 HVML 程序，通常意味着当前页面对应的 HVML 协程将被压制（suppressed），页面中的文档内容将被新的 HVML 协程覆盖。使用该页面名称时，将忽略页面分组以及页面类型信息。使用该保留名称时，不需要指定页面组和页面类型。
