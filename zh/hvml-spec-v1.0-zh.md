...
在完整的基于 HVML 的应用框架中，通常包含一个独立运行的用户界面渲染器。开发者通过撰写 HVML 程序来操控渲染器中的页面，而 HVML 程序在 HVML 解释器中运行，并可以和其他已有的编程语言的运行时环境结合在一起，接收由其他编程语言程序生成的数据，并按照 HVML 程序的指令，将其转换为图形用户界面的描述信息或者变更信息。通过这样的设计，我们将所有涉及到图形用户界面的应用程序分开成两个松散的模块：
