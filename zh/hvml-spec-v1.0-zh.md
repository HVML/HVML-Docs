...
- 类型不同的数据项不相等，但数值、有符号长整型、无符号长整型、长双精度浮点数，视作同一种类型，统称为数值。
- 两个字符串相同时，相等。
- 两个数值相同时，相等。
- 两个数组的成员一对一相同时，相等。
- 两个字典字符串化后的字符串相同时，相等。
...
- `casesensitively`：在 `init` 动作标签中初始化一个集合时，用于指定唯一性值的对比对大小写敏感，可简写为 `case`。
- `caseinsensitively`：在 `init` 动作标签中初始化一个集合时，用于指定唯一性值的对比对大小写不敏感，可简写为 `caseless`。
