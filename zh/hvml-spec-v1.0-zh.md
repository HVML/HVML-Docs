...
- 算术运算符：`+`、`-`、`*`、`/`、`%`、`//`、`**`；对参与运算的非数值类变体，执行数值化后再参与运算。注意类型提升。
- 比较运算符：`==`、`!=`、`>`、`>=`、`<`、`<=`；对非数值类变体，执行数值化后再进行比较。注意类型提升。
- 逻辑运算符：`not`、`and`、`or`；对于非布尔类变体，执行布尔化后再进行相应的运算。
- 成员运算符：`in`、`not in`；用于检测指定的键名是否存在于对象中。
- 位运算符：`&`、`|`、`~`、`^`、`<<`、`>>`；针对 `longint` 和 `ulongint` 类变体，其他类型执行数值化并转换为这两种类型之一再参与运算。
