...
通常，当某个动作元素被设置有 `silently` 副词属性，或者骨架元素被设置为 `hvml:silently` 属性时，对其属性、执行器、内容的求值时，若遇到可忽略异常，应返回一个合理的返回值，而不标记异常。

比如调用 `$SYSTEM.time(! <number $seconds: seconds since Epoch> )` 设置系统时间时，如果当前用户没有权限修改系统时间，通常应该产生 `AccessDenied` 异常。但如果在调用该方法的元素中，设置有 `silentyl` 副词属性，则不会产生异常，而是返回 `false` 标记出现错误。
...
明确了可忽略的异常，以及如何处理 `silently` 副词属性。
