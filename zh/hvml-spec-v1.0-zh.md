...
第一，一个和 GUI 无关的数据处理模块，开发者可以使用任何其熟悉的编程语言和开发工具开发这个模块。比如，涉及到人工智能处理时，开发者选择 Python；在 Python 代码中，除了装载 HVML 程序之外，开发者无需考虑任何和界面渲染及交互相关的东西，比如创建一个按钮或者点击一个菜单后完成某项工作等的这类工作，开发者只需要在 Python 代码中准备好渲染界面所需要的数据即可。我们将这些数据处理模块统称为“外部程序（external program）”。
...
1. `数据（data）`。
1. `变量（variable）`。
1. `数据项（data item）`或`数据成员（data member）`。对数组而言，每个数组单元就是一个数据项；对对象数据而言，其中的某个键值对就是一个数据项。为防止混淆，我们避免使用 `元素（element）` 一词来指代数据项或者数据成员。
...
1. HVML 解释器采用独特的变量和数据管理模型，既不需要开发者关心数据的分配和释放，也不需要在解释器中使用昂贵的垃圾收集器。
...
- 异常（exception）。该数据仅在内部使用，用来表示异步初始化变量时产生的异常。
...
除了内置的 `$SYSTEM` 动态对象之外，我们还可以通过外部程序来实现自定义的动态对象，并通过 `init` 标签将这个动态对象和某个变量绑定在一起，如：
...
当我们引用一个动态对象上并不存在的动态属性，或者不存在的虚拟子属性，或者无法在该属性上执行函数操作时，HVML 解释器或该对象的外部程序实现将返回错误或抛出异常。
...
- `$<`：指父操作的输入数据（input data），初始就是对介词属性 `on` 的属性值求值后的数据；迭代时可能会修改这个数据。
...
- `$<N><SYMB>`，如 `$1<`、 `$1?` 等：指从当前上下文向上回溯 `<N>` 级的上下文数据；这里的 `<N>` 必须是零或者正整数。这个上下文变量主要用于访问执行栈上祖先动作元素对应栈帧的上下文数据。当 `<N>` 为零时，指访问当前栈帧中的上下文变量。

有关当前栈帧中的上下文变量，说明如下：

1. 所有上下文变量初始定义为 `undefined`。访问未定义的上下文变量，会产生 `NoData` 异常。
1. 上下文变量的设置和某些属性有关，比如 `$0<` 初始来自 `on` 属性值，`$0@` 来自 `in` 属性值或者继承自父栈帧。因此，引用这些上下文变量时，要注意动作元素规定的属性值求值顺序。
...
`$TIMERS`：用于全局的定时器，具有固定的格式，初始为一个空的集合。可使用 `update` 等元素修改它的值，如：
...
- 其默认动作为在当前目标文档位置上插入（追加）该标签定义的元素。
...
- `error`：出现错误时，尝试用其中包含的内容插入到目标 DOM 树的当前位置。`error` 标签支持 `type` 属性，用来指定对应的错误类型。如：
...
- `except`：出现未被捕获的异常时，插入其中包含的内容到目标 DOM 树的当前位置。`except` 标签支持 `type` 属性，用来指定对应的异常类型。如：
   - 解析相关：
...
另外，对可应对的异常，HVML 提供了 `catch` 动作标签，可用来捕获特定的异常并进行处理。

错误和异常的处理说明如下：

1. 在 `error` 和 `except` 标签中，我们一般使用目标标记语言的标签定义一个文档片段，也称为错误和异常模板。
1. 当发生异常时，首先看当前元素是否含有对应的 `catch` 动作元素。若有，则执行该 `catch` 元素定义的操作组；若没有，则检查是否有对应的 `except` 子元素，若有，则克隆其中定义的文档片段并追加到目标文档的当前位置，弹出当前栈帧到父栈帧继续执行。如果异常未被当前元素处理，则弹出当前栈帧，在父栈帧中重复该步骤直到栈顶。
1. 当发生错误时，首先看当前元素是否含有对应的 `error` 子元素。若有，则克隆其中定义的文档片段并追加到目标文档的当前位置，直接弹出当前执行栈中的所有栈帧到栈顶。如果错误未被当前元素处理，则弹出当前栈帧，在父栈帧中重复该步骤直到栈顶。
...
1. 第一次迭代前：
   1. 使用 0 初始化 `$0%` 。
   1. 使用 `on` 属性值初始化 `$0<`。
   1. 使用 `in` 属性值或者继承父栈帧 `$@` 初始化 `$0@`。
1. 每次迭代前：
   1. 若不是第一次迭代（即 `$0%` 的值大于 0），且具有副词属性 `nosetotail`，则使用 `$0?` 值重置 `$0<`。
   1. 对 `onlyif` 属性定义的表达式求值，若结果为 `false`，则终止迭代，否则继续迭代。
1. 每次迭代中：
   1. 对 `with` 属性定义的表达式求值。
   1. 若结果为 `false`，则终止迭代，否则将结果设置到 `$0?` 上并处理子元素。
1. 每次迭代后：
   1. 对 `while` 属性定义的表达式求值。
   1. 若结果为 `false`，则终止迭代，否则`$0%` 增加 1，开始新的迭代。
...
1. 迭代的初始输入数据（也就是上下文变量 `$<` 的值）来自 `on` 属性，而在执行下次迭代之前，上下文变量 `$<` 的值将会用上次迭代的结果数据替代。
...
1. 当 `with` 属性的属性值是数值型数据，且修改动作为 `displace` 时：
   ...
   - `%=`：若指定的属性值大于零，则转换为正整数，将初始的最终数据做数值化处理，然后对最终数据作取模操作并用结果替代最终数据。
   - `~=`：将初始的最终数据做数值化处理，然后圆整为最接近的整数值替代最终数据（相当于浮点数的 `round` 操作）。
   - `^=`：将初始的最终数据做数值化处理，然后取不超过该数值的最大整数值替代最终数据（相当于浮点数的 `floor` 操作）。
   - `$=`：将初始的最终数据做数值化处理，然后取不小于该数值的最小整数值替代最终数据（相当于浮点数的 `ceil` 操作）。
...
在 `init` 标签中不指定 `as` 或者 `at` 属性时，则定义或者覆盖父栈帧中的用户自定义数据。
